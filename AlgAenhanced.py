...
#the nearest neighbour algorithm to get the initial best tour/length
...
def SA():
    ...
    successor = current
    #successor,successor_cost = nearestNeighbour()
    ...
    while t < 1000:
        ...
        while T > 0.1:
            ...
            if delta_E >= 0:
                ...
                successor,successor_cost = current,current_cost
                plateau = 0
            #increase number of times best length has not been found
            else:
                plateau = plateau + 1
            count +=1
            #trialling different temperature update formulas
            '''ENHANCEMENT START'''
            if formula == 1:           
                T = start_T/(1+(alpha*t))
            if formula == 2:
                T = start_T/(1+alpha*t**2)
            if formula == 3:
                T = T*alpha
            if formula == 4:
                new_T = T/(math.log(1+(plateaus+1)))
            if formula == 5:
                new_T = T*0.9 + 0.0001
            '''ENHANCEMENT END'''
