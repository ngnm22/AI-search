...
#swap function
def swap(s):
    #pick a random number in cities range for n 
    n = random.randint(0, len(s)-1)
    while True:
      #do the same for m but m cannot equal n
...
def SA():
    ...
    #calculate cost of random tour
    current_cost = weight(current)
    #update these above variables as the best tour/length
    successor_cost = current_cost
    successor = current
    #initialise schedule array
    schedule = []
    #intialize epsilon variable
    epsilon = 1
    #initialise iteration count
    t = 1
    #start a while loop
    while t > 0:
        #add new temperature to schedule
        schedule.append(T)
        #set new temperature
        T = schedule[t-1]/(1.0001)
        #return best length/tour if T is below epsilon
        if T < epsilon:
            return successor,successor_cost
        else:
            #swap two nodes
            current = swap(successor)
            #calculate cost of tour found
            current_cost = weight(current)
            #if current length is less than best length
            delta_E = current_cost - successor_cost
            if delta_E < 0:
                #update best length/tour to current ones
                successor,successor_cost = current,current_cost
            #if probability function is less than random
            elif prob(delta_E, T) < random.random():
                #also update best length/tour
                successor,successor_cost = current,current_cost
        #stop if time is almost larger than a minute
