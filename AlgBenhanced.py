...
############
#initialising alpha,beta,rho,number of ants
import math, random
from math import inf 
#using the nearest neighbour algorithm to get the initial best tour and best length
...
#Forming a matrix of heuristic desirabilities
...
def evaporation(pheromone_level,tmax,tmin):
    pheromone = [[ pheromone_level for i in range(num_cities) ] for j in range(num_cities)]
    evaporate = pheromone
    for i in range(num_cities):
        for j in range(i,num_cities):
            evaporate[i][j] = (1-rho)*(evaporate[i][j])
            evaporate[j][i] = (1-rho)*(evaporate[j][i])
            '''ENHANCEMENT START'''
            if evaporate[i][j] > tmax:
                evaporate[i][j] =tmax
            if evaporate[j][i] > tmax:
                evaporate[j][i] = tmax
            if evaporate[j][i] <tmin:
                evaporate[j][i] =tmin
            if evaporate[i][j] > tmin:
                evaporate[i][j] = tmin
            '''ENHANCEMENT END'''
    return pheromone,evaporate

#part of the second part of pheromone deposit/evaporate rule
def deposit(trail,cost,evaporate,k,vertex,L):
    #print(evaporate) 
    if vertex in trail:
        addition = 1/cost
    else:
...
def weight(tour): 
    ...
    cost += dist_matrix[tour[len(tour)-1]][tour[0]]
    return(cost)
# Reinitialise the pheromone matrix upon
'''ENHANCEMENT FUNCTIONS'''
def pheromone_trail_smoothing(pheromone, pts, initial_pheromone):
    for i in range(0, num_cities):
        for j in range(i, num_cities):
...
# Optimise the local minimum with two opt 
def two_opt(cost,trail):
    best = trail
    L = cost
...
#deciding when to use two opt when stagnating
def iteration(t):
    if t <= 25:
        return True
    elif 25 < t <= 75:
        return False if t % 5 == 0 else True
    elif 75 < t <= 150:
        return False if t % 3 == 0 else True
    elif 150 < t <= 250:
        return False if t % 2 == 0 else True
    else:
        return False

#find stagnation using formula
def stagnating(stag,tmax,tmin,pheromone,n):
    stag += min((tmax-pheromone),(pheromone-tmin))/(n**2)
    #stag = stag/(n**2)
    return stag
...
def ACO():
    ...
    startTime = time.time()
    #use nearest neighbour algorithm to intialise the best tour and length
    best,L = nearestNeighbour()
    #inialize pheromone deposit
    initial_deposit = num_cities/(L)
    #initialise iteration at:
    tmax = inf
    tmin = 0.0000000000000000000001
    t = 0
    #initialise max iterations
    max_it = 10000
    #intiliase probability for tmax and tmin
    stag_prob = 0.05
    stag = 0
    while t<max_it:
        #state initial pheromone and evaporation matrix         
        pheromone,evaporate = evaporation(initial_deposit,tmax,tmin)
        ...
        for k in range(1,num_cities):
            ...
            while(len(trail) < num_cities):
                ...
                vertex,probability = solution(trail,current,heuristic)
            ...
            if cost < L:
                ...
                L = cost
                best = trail
                #state tmax and tmin formulas (new every loop)
                '''ENHANCEMENT START'''
                tmax = num_ants/((1-rho)*L)
                tmp = stag_prob ** (1.0 / num_ants)
                tmin = tmax * (1 - tmp) / ((num_ants / 2. - 1) * tmp)
                var = 0
                '''ENHANCEMENT END'''
            else:
                var = var+1
            '''ENHANCEMENT START'''
            stag = stagnating(stag,tmax,tmin,pheromone[current][k],num_cities)
            #if a plateau is above 1000
    
            if var >1000:
                #stop before a minute
                if (time.time()) - startTime < 40:
                    #use two opt swap
                    trail,cost = two_opt(L,best)
                    #if the length is less then update
                    if cost < L:
                        L,best = cost, trail
                    #reset 
                    var = 0
                    pheromone = pheromone_trail_smoothing(pheromone, 1, initial_deposit)
            if iteration(t) == False:
                chosen = L
                chosen2 = best
            else:
                chosen = cost
            ...
            '''ENHANCEMENT END'''
            #deposit function
            depositing = deposit(chosen2,chosen,evaporate,k-1,vertex,L)
